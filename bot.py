...
def unique_users(message):
    text = "Количество пользователей: " + str(analise())
...
def all_message(message):
    try:
        global check
        check = message.text
        if message.text == "Старт" or message.text == "старт":
            statistics(message.chat.id)
            ...
            bot.send_message(message.chat.id, text="Предлагаю тебе определиться с нашим маршрутом. Я," +
        ...
        if message.text == "улица Рождественка":
            ...
            bot.send_message(message.chat.id, text=message.text, reply_markup=keyboard, parse_mode="Markdown")
        ...
        # end
            ...
            bot.send_message(message.chat.id, text="Спасибо, что составил мне компанию! Надеюсь, тебе было" +
